...
namespace Log
{
	LOG getlog(wchar_t logfile[])//Функция для получения структуры LOG. 
	{
	...
	void WriteLine(LOG log, char* c, ...)//Функция для записи строки в файл протокола.
	{
		char **p = &c;
	...
	void WriteLine(LOG log, wchar_t* c, ...)//Функция для записи строки в файл протокола.
	{
		wchar_t **p = &c;
		char buffer[50];
	...
	void WriteLog(LOG log)//Функция для записи информации о дате и времени в файл протокола.
	{
	...
	void WriteParm(LOG log, Parm::PARM parm)//Функция для записи информации о параметрах программы в файл протокола. 
	{
	...
	void WriteIn(LOG log, In::IN in)//Функция для записи исходных данных в файл протокола.
	{
	...
	void WriteError(LOG log, Error::ERROR error)//Функция для записи информации об ошибке в файл протокола.
	{
		if (error.id == 100)
		{
	...
	void WriteLexTableLog(LT::LexTable& lextable, std::ostream& out)//Функция для записи таблицы лексем в файл протокола.
	{
		int i;
